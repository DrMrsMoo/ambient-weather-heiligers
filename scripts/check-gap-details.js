...
async function checkGapDetails() {
  ...
  console.log(`  Actually in cluster: ${period1Result.count} records`);
  console.log(`  Status: ${period1Result.count > 0 ? '✓ HAS DATA' : '✗ MISSING'}`);
  console.log();
  ...
  console.log(`  Actually in cluster: ${period2Result.count} records`);
  console.log(`  Status: ${period2Result.count > 100 ? '✓ HAS DATA' : '✗ MISSING DATA'}`);
  console.log();
  ...
  if (lastBefore.hits.hits.length > 0) {
    console.log(`  Last doc before gap: ${lastBefore.hits.hits[0]._source.date}`);
  }
  if (firstAfter.hits.hits.length > 0) {
    console.log(`  First doc after gap: ${firstAfter.hits.hits[0]._source.date}`);
  }
